...
class Decoder(nn.Module):
    def __init__(self,C_hid, C_out, N_S):
        ...
        self.readout = nn.Conv2d(704, 64, 1)
    ...
    def forward(self, hid, enc1=None):
        ...
        ys =Y.shape
        print("ys: ",ys)
        Y = Y.reshape(int(ys[0]/11), int(ys[1]*11), 160, 240)
        Y = self.readout(Y)
        return Y
...
class IAM4VP(nn.Module):
    def __init__(self, shape_in, hid_S=64, hid_T=512, N_S=4, N_T=6):
        ...
        self.readout = nn.Conv2d(64, 3, 1)
        self.mask_token = nn.Parameter(torch.zeros(11, hid_S, 40, 60))
    ...
    def forward(self, x_raw, y_raw=None, t=None):
        ...
        print("after encode:",embed.shape)
        ...
        print("z shape after cat:",z.shape)
        hid = self.hid(z, time_emb)
        print("shape after hid:",hid.shape)
        hid = hid.reshape(B*T, C_, H_, W_)
        print("reshape after hid:",hid.shape)

        Y = self.dec(hid, skip)
        print("after dec:",Y.shape)
        Y = self.attn(Y)
        print("after attn:",Y.shape)
        Y = self.readout(Y)
        print("after readout:",Y.shape)
        return Y

if __name__ == "__main__":
    import numpy as np
    model = IAM4VP([11,3,160,240])
    inputs = torch.randn(2,11,3,160,240)
    inputs2 = torch.randn(2,11,3,160,240)
    pred_list = []
